...
use super::parser::{parse_basic_resource, parse_complex_resource, parse_explorer_kind, parse_id};
    ...
    loop {
        ...
        let parsed_command: Result<Option<UiToOrchestratorCommand>, String> = match cmd.as_str() {
            ...
            "generate" | "gen" => match parts.as_slice() {
                ...
                _ => Err(
                    "Usage: generate|gen <explorer_id> <carbon|hydrogen|oxygen|silicon>"
                        .to_string(),
                ),
            },
